...
def extract_all_dots_opencv(
    ...
    filtered_slice_ranges = []
    kept_indices = []

    for i, dots in enumerate(observations):
        if len(dots) == 0:
            continue
        filtered_observations.append(dots)
        filtered_slice_ranges.append(slice_ranges[i])
        kept_indices.append(i)

    print("\n--- Slice 0 dots ---")
    for i, d in enumerate(filtered_observations[0]):
        print(f"[{i}] x={d['x']:.2f}, y={d['y']:.2f}")

    print("\n--- Slice 1 dots ---")
    for i, d in enumerate(filtered_observations[1]):
        print(f"[{i}] x={d['x']:.2f}, y={d['y']:.2f}")
    return filtered_observations, filtered_slice_ranges, all_dots
...
def pairwise_cost_with_curvature(
    y_prev: float,
    dot_curr: dict,
    dot_next: dict,
    ...
    """
    Cost of extending a track:
        previous dot  -> current dot -> candidate next dot
    ...
    curvature:
        (y_next - 2*y_curr + y_prev)^2
    """
    y_curr = float(dot_curr["y"])
    y_next = float(dot_next["y"])

    dist_cost = abs(y_next - y_curr)
    curv_cost = (y_next - 2.0 * y_curr + y_prev) ** 2

    da = abs(dot_curr.get("area", 1) - dot_next.get("area", 1))
    dh = abs(dot_curr.get("height", 1) - dot_next.get("height", 1))

    return (
        dist_weight * dist_cost
        + curv_weight * curv_cost
        + area_weight * da
        + height_weight * dh
    )
...
def match_adjacent_slices(
    ...
    for i, da in enumerate(dots_a):
        ...
        # predecessor of the current node, if it exists
        prev_node = predecessor.get(node_a, None)
        dot_prev = None if prev_node is None else observations[prev_node[0]][prev_node[1]]

        for j, db in enumerate(dots_b):
            dx = abs(float(db["x"]) - float(da["x"]))
            dy = abs(float(db["y"]) - float(da["y"]))

            # allow larger y change when the two non-empty slices are farther apart
            adaptive_max_dy = max_dy * max(dx, 1.0)

            if dy <= adaptive_max_dy:
                cost = pairwise_cost_with_curvature(
                    dot_prev=dot_prev,
                    dot_curr=da,
                    dot_next=db,
                ...
                # optional normalization by dx if filtered slices are unevenly spaced
                cost_matrix[i, j] = cost / max(dx, 1.0)
...
def build_track_graph(
    ...
    """
    Build a graph of vessel tracks using a curvature-aware extension rule.

    Strategy
    --------
    1. Create all nodes.
    2. Initialize edges between slice 0 and slice 1 using distance only.
    3. For slice k >= 2, extend each existing node in slice k-1 using:
           distance + curvature
       where curvature depends on the matched predecessor in slice k-2.

    Returns
    -------
    nodes, edges, components, node_to_component
    """
    nodes = {}
    edges = []
    uf = UnionFind()
    ...
    if len(observations) == 1:
        components = [[node_id] for node_id in nodes]
        node_to_component = {node_id: i for i, node_id in enumerate(nodes)}
        return nodes, edges, components, node_to_component

    # --------------------------------------------------------
    # B. First slice pair: initialize with distance-only matching
    # --------------------------------------------------------
    prev_match = {}  # maps node in slice k to its predecessor in slice k-1

    initial_matches = match_adjacent_slices(
        observations[0],
        observations[1],
        max_dy=max_dy,
        area_weight=area_weight,
    ...
    for i0, i1, cost in initial_matches:
        n0 = (0, i0)
        n1 = (1, i1)
        edges.append((n0, n1, cost))
        uf.union(n0, n1)
        prev_match[n1] = n0

    # --------------------------------------------------------
    # C. Extend tracks using curvature from slice 2 onward
    # --------------------------------------------------------
    for s_idx in range(2, len(observations)):
        dots_next = observations[s_idx]

        used_next = set()
        new_prev_match = {}

        # Only nodes in slice s_idx - 1 that already belong to a track
        curr_nodes = [node_id for node_id in prev_match.keys() if node_id[0] == s_idx - 1]

        for curr_node in curr_nodes:
            prev_node = prev_match[curr_node]

            dot_prev = observations[prev_node[0]][prev_node[1]]
            dot_curr = observations[curr_node[0]][curr_node[1]]
            y_prev = float(dot_prev["y"])
            y_curr = float(dot_curr["y"])

            best_j = None
            best_cost = float("inf")

            for j, dot_next in enumerate(dots_next):
                if j in used_next:
                    continue

                y_next = float(dot_next["y"])
                if abs(y_next - y_curr) > max_dy:
                    continue
            ...
            if best_j is not None:
                next_node = (s_idx, best_j)
                edges.append((curr_node, next_node, best_cost))
                uf.union(curr_node, next_node)
                used_next.add(best_j)
                new_prev_match[next_node] = curr_node

        prev_match = new_prev_match

    # --------------------------------------------------------
    # D. Extract connected components
    # --------------------------------------------------------
    root_to_nodes = {}
